...
class SOSCalibratedPricingCalculator:
    ...
    def __init__(self):
        ...
        self.calibration_multipliers = {"POR": 1.0, "DIF": 1.0, "CEN": 1.0, "ATT": 1.0}
    ...
    def match_with_sos(
        self, merged_df: pd.DataFrame, sos_df: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        logger.info(
            f"Trovati {matches_found} match su {len(merged_df)} giocatori totali"
        )
        ...
        if matches_found > 0:
            ...
            for _, row in matched_examples.iterrows():
                logger.info(
                    f"  {row['Nome']} ({row['Surname']}) -> SOS Prezzo: {row['Prezzo']}€"
                )
    ...
    def calculate_comprehensive_score(
        self, player_row: pd.Series, role: str
    ) -> Dict[str, float]:
        """
    ...
    def calculate_calibrated_price(
        self, player_row: pd.Series, scores: Dict[str, float]
    ) -> float:
        """
        ...
        if sos_price is None or sos_price <= 0:
            ...
            base_multiplier = {"POR": 2.0, "DIF": 2.5, "CEN": 3.0, "ATT": 4.0}[role]
    ...
    def process_data(self, merged_file: str, sos_file: str, output_file: str):
        ...
        final_cols = [
            col for col in priority_cols if col in result_df.columns
        ] + remaining_cols
