...
class FantacalcioWorkflow:
    ...
    def _merge_dataframes(self, df_fpedia: pd.DataFrame, df_fstats: pd.DataFrame) -> pd.DataFrame:
        ...
        def fuzzy_match_names(fpedia_names, fstats_names, threshold=0.75):
            ...
            for fp_name in fpedia_names:
                ...
                available_fstats = [name for name in fstats_names if name not in used_fstats]
                best_matches = difflib.get_close_matches(fp_name, available_fstats, n=1, cutoff=threshold)
            ...
            for fp_name in fpedia_names:
                ...
                for fs_name in fstats_names:
                    ...
                    similarity = difflib.SequenceMatcher(None, fp_inverted, fs_name).ratio()
                    if similarity >= 0.8:
                        matches[fp_name] = fs_name
                        used_fstats.add(fs_name)
                        break
            ...
            for fp_name in fpedia_names:
                ...
                for fs_name in fstats_names:
                    ...
                    for fs_part in fs_parts:
                        if difflib.SequenceMatcher(None, fp_surname, fs_part).ratio() > 0.85:
                            surname_match = True
                            break
                    
                    if surname_match:
                        # Controlla similarità generale
                        overall_similarity = difflib.SequenceMatcher(None, fp_name, fs_name).ratio()
        ...
        print(f"   FPEDIA examples: {df_fpedia_clean[['Nome', 'Nome_Clean']].head(3).to_dict('records')}")
        print(f"   FSTATS examples: {df_fstats_clean[['Nome', 'Nome_Clean']].head(3).to_dict('records')}")
        ...
        if fuzzy_matches:
            ...
            for i, (fp_clean, fs_clean) in enumerate(list(fuzzy_matches.items())[:3]):
                fp_orig = df_fpedia_clean[df_fpedia_clean['Nome_Clean'] == fp_clean]['Nome'].iloc[0]
                fs_orig = df_fstats_clean[df_fstats_clean['Nome_Clean'] == fs_clean]['Nome'].iloc[0]
        ...
        for fp_clean, fs_clean in fuzzy_matches.items():
            ...
            df_fstats_clean.loc[df_fstats_clean['Nome_Clean'] == fs_clean, 'Nome_Clean'] = fp_clean
    ...
    def _clean_empty_rows(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if before != after:
            print(f"   🧹 Rimosse {before - after} righe con dati insufficienti ({both_sources.sum()} matchati + {(sufficient_data & ~both_sources).sum()} con dati sufficienti)")
    ...
    def _calculate_combined_scores(self, df_merged: pd.DataFrame) -> pd.DataFrame:
        ...
        df_merged['Performance_Score'] = (df_merged['Convenienza'].fillna(0) * 20).round(2)
        ...
        if 'Prezzo_Massimo_Consigliato_FPEDIA' in df_merged.columns and 'Prezzo_Massimo_Consigliato_FSTATS' in df_merged.columns:
    ...
    def _merge_with_sos_data(self, df_merged: pd.DataFrame, df_sos: pd.DataFrame) -> pd.DataFrame:
        """Merge con i dati SOS Fanta."""
        # Semplice merge per nome (assume colonna Nome in entrambi)
        df_final = pd.merge(df_merged, df_sos, on='Nome', how='left', suffixes=('', '_SOS'))
        ...
        if 'Prezzo_SOS' in df_final.columns:
            ...
            # Calcola differenza
            df_final['Differenza_SOS'] = df_final['Prezzo_Consigliato'] - df_final['Prezzo_Mercato']
...
Esempi di utilizzo:
    ...
    parser.add_argument(
        '--update-data',
        action='store_true',
        help='Aggiorna i dati FPEDIA e FSTATS'
    )
